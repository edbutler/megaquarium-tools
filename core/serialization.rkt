...
(define (serialize val)
  (cond
   ...
   [(set-equal? val) `(set ,(map serialize (set->list val)))]
...
(define (deserialize val)
  (match val
   ...
   [`(set ,val) (list->set (map deserialize val))]
...
(define (serialize-to-file fname obj)
  (call-with-output-file fname #:exists 'truncate (λ (out) (write (serialize obj) out))))

(define (deserialize-from-file fname)
  (deserialize (call-with-input-file fname (λ (in) (read in)))))
